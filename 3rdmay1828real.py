...
#normalised_df_new = normalised_df_new[:325]
#MJD_new = MJD_new[:325]
...
pca_even =createeven_df_andtranspose(MJD_new, evenly_spaced_mjd_new, PCA_matrix)
...
'''
dataa = normalised_df_new.iloc[:, np.r_[360:660]]
column_av = dataa.mean(axis=0)
Wmode = dataa.iloc[75,:]
Nmode = dataa.iloc[77,:]
...
for i in range(0,300):
    ...
    plt.title(f'B1822-09 one profile {i}')
    plt.show()
'''
fig, axes = plt.subplots(5, 1, figsize=(8, 6), gridspec_kw={'height_ratios': [1,0.1,1, 0.5, 0.5]})

extent = [evenly_spaced_mjd_new[0],evenly_spaced_mjd_new[len(evenly_spaced_mjd_new)-1],1,60]
...
axes[2].imshow(pca_even, cmap='inferno', aspect='auto', extent = extent)
...
axes[4].set_xlabel('Modified Julian Days')  # Replace with appropriate label
axes[4].set_ylabel('spin-down')  # Replace with appropriate label
axes[4].set_xlim(evenly_spaced_mjd_new[0], evenly_spaced_mjd_new[-1])
...
SP1_res_new = resample(spindown_MJD_new, MJD_new, shape_parameter_new)

SP2_res_new = resample(spindown_MJD_new, MJD_new, shape_parameter_new1)

SP3_res_new = resample(spindown_MJD_new, MJD_new, shape_parameter_new2)

SP4_res_new = resample(spindown_MJD_new, MJD_new, shape_parameter_new3)
...
axes[1].plot(spindown_MJD_new, SP1_res_new, color='red')
...
axes[2].plot(spindown_MJD_new, spindown_rate_new, color='red')
...
#srcc1_new,pvalue1, uncertainty1_new = calc_srcc(spindown_rate_new,SP1_res_new)

#srcc2_new,pvalue2, uncertainty2_new = calc_srcc(spindown_rate_new,SP2_res_new)

#srcc3_new,pvalue3, uncertainty3_new = calc_srcc(spindown_rate_new,SP3_res_new)

#srcc4_new,pvalue4, uncertainty4_new = calc_srcc(spindown_rate_new,SP4_res_new)
...
#print(f'{SP_1_2_CORR_new}±{unc12_new}')

#print(f'{SP_2_3_CORR_new}±{unc23_new}')

smoothed_MJD_new, smoothed_shape_parameter_new = smooth_sp_and_days(MJD_new, shape_parameter_new, window)

smoothed_MJD_new, smoothed_shape_parameter_new1 = smooth_sp_and_days(MJD_new, shape_parameter_new1, window)

smoothed_MJD_new, smoothed_shape_parameter_new2 = smooth_sp_and_days(MJD_new, shape_parameter_new2, window)

smoothed_MJD_new, smoothed_shape_parameter_new3 = smooth_sp_and_days(MJD_new, shape_parameter_new3, window)
...
print(f'{srcc1_new}±{uncertainty1_new}, pvalue{pvalue1}')

print(f'{srcc2_new}±{uncertainty2_new}, pvalue{pvalue2}')

print(f'{srcc3_new}±{uncertainty3_new}, pvalue{pvalue3}')

print(f'{srcc4_new}±{uncertainty4_new}, pvalue{pvalue4}')


nudot_resampled_smooth = resample_using_cubic_spline(spindown_MJD_new, spindown_rate_new , smoothed_MJD_new)

srcc1_new,pvalue1, uncertainty1_new = calc_srcc(nudot_resampled_smooth, smoothed_shape_parameter_new)

srcc2_new,pvalue2, uncertainty2_new = calc_srcc(nudot_resampled_smooth, smoothed_shape_parameter_new1)

srcc3_new,pvalue3, uncertainty3_new = calc_srcc(nudot_resampled_smooth, smoothed_shape_parameter_new2)

srcc4_new,pvalue4, uncertainty4_new = calc_srcc(nudot_resampled_smooth, smoothed_shape_parameter_new3)
...
axes[1].plot(MJD_new, shape_parameter_new1, color='red')
axes[1].set_ylabel(f'resampled shape parameter')
axes[1].set_xlim(evenly_spaced_mjd_new[0], evenly_spaced_mjd_new[-1])
axes[1].xaxis.set_visible(False)

# Plot the spindown dfb on the second subplot
axes[2].plot(MJD_new, shape_parameter_new2, color='red')
axes[2].set_ylabel(f'spindown rate')
axes[2].set_xlim(evenly_spaced_mjd_new[0], evenly_spaced_mjd_new[-1])
axes[2].xaxis.set_visible(False)

axes[3].plot(MJD_new, shape_parameter_new3, color='red')
axes[3].set_ylabel(f'spindown rate')
axes[3].set_xlim(evenly_spaced_mjd_new[0], evenly_spaced_mjd_new[-1])
axes[3].xaxis.set_visible(False)

axes[4].plot(MJD_new, nudot_resampled, color='red')
axes[4].set_ylabel(f'spindown rate')
axes[4].set_xlim(evenly_spaced_mjd_new[0], evenly_spaced_mjd_new[-1])
axes[4].xaxis.set_visible(False)

axes[5].plot(spindown_MJD_new, spindown_rate_new, color='red')
axes[5].set_ylabel(f'spindown rate')
axes[5].set_xlim(evenly_spaced_mjd_new[0], evenly_spaced_mjd_new[-1])
...
fig, ax = plt.subplots()
ax.plot(spindown_MJD_new, spindown_rate_new, color='red')
ax.set_xlabel('Modified Julian Days')
ax.set_ylabel(r'$\dot{\nu}$')
ax.set_xlim(evenly_spaced_mjd_new[0], evenly_spaced_mjd_new[-1])

for spine in ax.spines.values():
    if spine == ax.spines['top'] or spine == ax.spines['right']:
        spine.set_visible(False)

ax.tick_params(axis='both', which='both', bottom=True, top=False, left=True, right=False)

plt.tight_layout()  # Adjust layout to fit into figure area
plt.show()